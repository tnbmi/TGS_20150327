...
// コンストラクタ
//=============================================================================
CMeshDome::CMeshDome(int priority, OBJTYPE objType) : CScene(priority, objType)
{
	m_tex		= NULL;
	m_vtxBuff	= NULL;
	m_indexBuff	= NULL;
...
// 生成
//=============================================================================
CMeshDome* CMeshDome::Create(LPDIRECT3DDEVICE9 device, CImport::TEXTURES texture, D3DXVECTOR3 sizeBlock, D3DXVECTOR3 numBlock)
{
	CMeshDome* mesh= new CMeshDome;
...
// 初期化
//=============================================================================
HRESULT CMeshDome::Init(LPDIRECT3DDEVICE9 device, CImport::TEXTURES texture, D3DXVECTOR3 sizeBlock, D3DXVECTOR3 numBlock)
{
	//----------------------------
	// デバイス取得
	//----------------------------
...
// 頂点情報設定
//=============================================================================
void CMeshDome::SetVertexPolygon(void)
{
	VERTEX_3D* vtx;
	int			workVtx;
	float		workAngleX = D3DX_PI * 2.0f / m_numBlock.x;
	float		workAngleY = D3DX_PI / (m_numBlock.y * 2.0f);
	...
	for(float cntY = 0; cntY <= m_numBlock.y; ++cntY)
	{
		...
		for(float cntX = 0; cntX <= m_numBlock.x; ++cntX)
		{
			...
			// 頂点座標
			vtx[workVtx].vtx.x = cos(workAngleX * cntX) * cos(workAngleY * cntY) * m_size.x;
			vtx[workVtx].vtx.z = sin(workAngleX * cntX) * cos(workAngleY * cntY) * m_size.z;
			vtx[workVtx].vtx.y = sin(workAngleY * cntY) * m_size.y;
			...
			// テクスチャ座標
			vtx[workVtx].tex = D3DXVECTOR2(texcord.x * (float)cntX, 1.0f - texcord.y * (float)cntY);
		}
	}
...
// 頂点インデックス設定
//=============================================================================
void CMeshDome::SetVertexIndexPolygon(void)
{
	WORD*	index;
